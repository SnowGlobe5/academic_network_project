...
if os.path.exists(f"{ROOT}/processed/next_topic_edge{YEAR}.pt"):
    ...
    generate_next_topic_edge_year(data, YEAR)
...
embedding_author = torch.nn.Embedding(data["author"].num_nodes, 32)
embedding_topic = torch.nn.Embedding(data["topic"].num_nodes, 32)
...
def train():
    ...
    for i, batch in enumerate(tqdm(train_loader)):
        ...
        pred = model(train_data.x_dict, train_data.edge_index_dict,
                    train_data['author', 'topic'].edge_label_index)
        target = train_data['author', 'topic'].edge_label
...
def test(loader):
    ...
    for i, batch in enumerate(tqdm(loader)):
        ...
        pred = model(val_data.x_dict, val_data.edge_index_dict,
                     val_data['author', 'topic'].edge_label_index)
        pred = pred.clamp(min=0, max=1)
        target = val_data['author', 'topic'].edge_label.float()
