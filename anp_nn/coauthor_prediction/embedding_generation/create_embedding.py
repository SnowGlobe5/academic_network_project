...
cache_path = 'author_data_co_author_with_neg.pt'

if os.path.exists(cache_path):
    # Load existing processed data
    author_data = data
    author_data['author', 'co_author', 'author'].edge_index = torch.load(cache_path).edge_index
    author_data['author', 'co_author', 'author'].edge_label = torch.load(cache_path).edge_label
    ...
    torch.save(author_data['author', 'co_author', 'author'], cache_path)

autor_data_filtered = anp_simple_filter_data(author_data, root=ROOT, folds=None, max_year=YEAR)
...
embedding_path = f"../anp_data/processed/embeddings/author_embeddings_{YEAR}_{infosphere_type}_{infosphere_parameters}.pt"
if os.path.exists(embedding_path):
    ordered_embeddings = torch.load(embedding_path)
else:
    ordered_embeddings = generate_author_embeddings(model, author_loader).cpu().detach()
    torch.save(ordered_embeddings, embedding_path)
    exit()

print(f"Embeddings shape: {ordered_embeddings.shape}")
autor_data_filtered['author'].x = ordered_embeddings.to(DEVICE)

# Prepare validation data
val_data = anp_simple_filter_data(autor_data_filtered, root=ROOT, folds=[4], max_year=YEAR)
...
def validate():
    ...
    for batch in tqdm(val_loader):
        ...
        dict_author = {
            'author': batch['author'].x,
        }
        pred = model.decoder(dict_author, edge_label_index)
        target = edge_label
