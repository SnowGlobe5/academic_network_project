...
train_loader = NeighborLoader(
    ...
    num_neighbors=[edge_number, -1, 10],
...
embedding_topic = torch.nn.Embedding(data["topic"].num_nodes, 64).to(DEVICE)
...
def train():
    ...
    for batch in tqdm(train_loader):
        ...
        loss = F.poisson_nll_loss(pred, target, log_input=False)  # Use Poisson NLL Loss
        loss.backward()
        optimizer.step()
